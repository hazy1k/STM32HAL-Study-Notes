...
# 14 "../User/BSP\\bsp_init.h" 2
# 1 "../Middlewares\\SerialInvoker.h" 1
# 26 "../Middlewares\\SerialInvoker.h"
typedef unsigned char uint8_t;
typedef unsigned short uint16_t;
typedef unsigned int uint32_t;
# 38 "../Middlewares\\SerialInvoker.h"
...
struct _m_serial_invoker_dev {
    struct _m_serial_invoker_nametab *funs;
...
# 15 "../User/BSP\\bsp_init.h" 2

void bsp_init(void);
# 2 "../User/main.c" 2
# 1 "../User/BSP/sdio\\sdio.h" 1
...
int main(void)
{
  ...
  LCD_ShowNum(30+13*8, 150, (((uint64_t)((&sdcard_handle)->SdCard.LogBlockNbr) * ((&sdcard_handle)->SdCard.LogBlockSize)) >> 20),5,16);
 while(1)
  {
...
void show_sd_info(void)
{
  ...
  {
    case 0x00000000U:
      if(sdcard_info_handle.CardVersion == 0x00000000U)
      {
        printf("Card Type:SDSC V1\r\n");
      }
      else if(sdcard_info_handle.CardVersion == 0x00000001U)
      {
  ...
  printf("Card RCA:%d\r\n", sdcard_info_handle.RelCardAdd);
  printf("LogBlockNbr:%d \r\n", (uint32_t)(sdcard_info_handle.LogBlockNbr));
  printf("LogBlockSize:%d \r\n", (uint32_t)(sdcard_info_handle.LogBlockSize));
  printf("Card Capacity:%d MB\r\n", (uint32_t)(((uint64_t)((&sdcard_handle)->SdCard.LogBlockNbr) * ((&sdcard_handle)->SdCard.LogBlockSize)) >> 20));
  printf("Card BlockSize:%d\r\n\r\n", sdcard_info_handle.BlockSize);
}
...
void test_sd_read(uint32_t addr, uint32_t cnt)
{
  ...
  uint8_t *buf;
  uint8_t sta = 0;
  buf = mymalloc(0, cnt * 512);
  sta = sd_read_disk(buf, addr, cnt);
  if(sta == 0)
  {
...
void test_sd_write(uint32_t addr, uint32_t cnt)
{
  ...
  sta = sd_write_disk(buf, addr, cnt);
  if(sta == 0)
  {
    printf("Write over\r\n");
  }
