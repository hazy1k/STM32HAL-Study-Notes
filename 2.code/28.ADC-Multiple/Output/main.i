...
uint16_t g_adc_dma_buffer[(50 * 6)];
...
int main(void)
{
    ...
    adc_multi_channel_dma_enable((50 * 6));
    ...
    while (1)
    {
        if(g_adc_dma_transfer_complete_flag)
        {
            ...
            for (uint8_t channel_index = 0; channel_index < 6; channel_index++)
            {
                ...
                for (uint16_t sample_index = 0; sample_index < 50; sample_index++)
                {
                    channel_sum += g_adc_dma_buffer[sample_index * 6 + channel_index];
                }
                adc_raw_value = channel_sum / 50;
            ...
            g_adc_dma_transfer_complete_flag = 0;
        }
