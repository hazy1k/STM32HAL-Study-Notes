# 1 "../User/BSP/dac/dac.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 387 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../User/BSP/dac/dac.c" 2
...
void dac_init(uint8_t outx)
{
    do { volatile uint32_t tmpreg = 0x00U; ((((RCC_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x3800UL))->APB1ENR) |= ((0x1UL << (29U)))); tmpreg = ((((RCC_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x3800UL))->APB1ENR) & ((0x1UL << (29U)))); (void)tmpreg; } while(0U);
    ...
    GPIO_InitStruct.Pin = (outx==1)? ((uint16_t)0x0010) : ((uint16_t)0x0020);
    ...
    dac_handle.Instance = ((DAC_TypeDef *) (0x40000000UL + 0x7400UL));
    HAL_DAC_Init(&dac_handle);
    DAC_ChannelConfTypeDef dac_ch_handle;
    dac_ch_handle.DAC_Trigger = 0x00000000UL;
    dac_ch_handle.DAC_OutputBuffer = ((0x1UL << (1U)));

    switch (outx)
    {
    case 1:
        HAL_DAC_ConfigChannel(&dac_handle, &dac_ch_handle, 0x00000000U);
        HAL_DAC_Start(&dac_handle, 0x00000000U);
        break;

    case 2:
        HAL_DAC_ConfigChannel(&dac_handle, &dac_ch_handle, 0x00000010U);
        HAL_DAC_Start(&dac_handle, 0x00000010U);
        break;
    default:
        break;
    }
...
void dac_set_voltage(uint8_t outx, uint16_t voltage)
{
    ...
    if(outx)
    {
        HAL_DAC_SetValue(&dac_handle, 0x00000000U, 0x00000000U, temp);
    }
    else
    {
        HAL_DAC_SetValue(&dac_handle, 0x00000010U, 0x00000000U, temp);
    }
}
# 71 "../User/BSP/dac/dac.c"
void dac_triangular_wave(uint16_t maxval, uint16_t dt, uint16_t samples, uint16_t n)
{
    uint16_t i, j;
    float incval;
    float Curval;
    if((maxval + 1) <= samples) return ;
    incval = (maxval + 1) / (samples / 2);
    for(j = 0; j < n; j++)
    {
        Curval = 0;
        HAL_DAC_SetValue(&dac_handle, 0x00000000U, 0x00000000U, Curval);
...
void dac_dma_init(uint8_t outx)
{
    ...
    dac_handle.Instance = ((DAC_TypeDef *) (0x40000000UL + 0x7400UL));
    HAL_DAC_Init(&dac_handle);
    dac_dma_handle.Instance = (outx==1)?((DMA_Stream_TypeDef *) (((0x40000000UL + 0x00020000UL) + 0x6000UL) + 0x088UL)):((DMA_Stream_TypeDef *) (((0x40000000UL + 0x00020000UL) + 0x6000UL) + 0x0A0UL));
    dac_dma_handle.Init.Channel = 0x0E000000U;
    dac_dma_handle.Init.Direction = ((uint32_t)(0x1UL << (6U)));
    dac_dma_handle.Init.PeriphInc = 0x00000000U;
    dac_dma_handle.Init.MemInc = ((uint32_t)(0x1UL << (10U)));
    dac_dma_handle.Init.PeriphDataAlignment = ((uint32_t)(0x1UL << (11U)));
    dac_dma_handle.Init.MemDataAlignment = ((uint32_t)(0x1UL << (13U)));
    dac_dma_handle.Init.Mode = ((uint32_t)(0x1UL << (8U)));
    dac_dma_handle.Init.Priority = ((uint32_t)(0x1UL << (16U)));
    dac_dma_handle.Init.FIFOMode = 0x00000000U;
    HAL_DMA_Init(&dac_dma_handle);
    do{ (&dac_handle)->DMA_Handle1 = &(dac_dma_handle); (dac_dma_handle).Parent = (&dac_handle); } while(0U);
    dac_ch_handle.DAC_Trigger = ( (0x2UL << (3U)) | (0x1UL << (2U)));
    dac_ch_handle.DAC_OutputBuffer = 0x00000000U;
    HAL_DAC_ConfigChannel(&dac_handle, &dac_ch_handle, 0x00000000U);
}
# 140 "../User/BSP/dac/dac.c"
void dac_dma_wave_enable(uint8_t outx, uint16_t ndtr, uint16_t arr, uint16_t psc)
{
    ...
    tim7_handle.Instance = ((TIM_TypeDef *) (0x40000000UL + 0x1400UL));
    tim7_handle.Init.Prescaler = psc;
    tim7_handle.Init.CounterMode = 0x00000000U;
    tim7_handle.Init.Period = arr;
    tim7_handle.Init.AutoReloadPreload = (0x1UL << (7U));
    HAL_TIM_Base_Init(&tim7_handle);

    master_config.MasterOutputTrigger = (0x2UL << (4U));
    master_config.MasterSlaveMode = 0x00000000U;
    HAL_TIMEx_MasterConfigSynchronization(&tim7_handle, &master_config);
    HAL_TIM_Base_Start(&tim7_handle);
    HAL_DAC_Stop_DMA(&dac_handle, (outx == 1) ? 0x00000000U : 0x00000010U);
    HAL_DAC_Start_DMA(&dac_handle, (outx == 1) ? 0x00000000U : 0x00000010U, (uint32_t*)dac_sin_buf, ndtr, 0x00000000U);
}
