...
static void tim_cap_init(uint32_t arr, uint16_t psc)
{
    ...
    tim_cap_handler.Instance = ((TIM_TypeDef *) (0x40000000UL + 0x0000UL));
    tim_cap_handler.Init.Prescaler = psc;
    tim_cap_handler.Init.CounterMode = 0x00000000U;
    tim_cap_handler.Init.Period = arr;
    tim_cap_handler.Init.ClockDivision = 0x00000000U;
    HAL_TIM_IC_Init(&tim_cap_handler);


    tim_cap_ic_init.ICPolarity = 0x00000000U;
    tim_cap_ic_init.ICSelection = (0x1UL << (0U));
    tim_cap_ic_init.ICPrescaler = 0x00000000U;
    tim_cap_ic_init.ICFilter = 0x8;
    HAL_TIM_IC_ConfigChannel(&tim_cap_handler, &tim_cap_ic_init, 0x00000000U);


    HAL_TIM_IC_Start(&tim_cap_handler, 0x00000000U);
}


void tim_cap_deinit(void)
{
    HAL_TIM_IC_Stop(&tim_cap_handler, 0x00000000U);
    HAL_TIM_IC_DeInit(&tim_cap_handler);
    (((RCC_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x3800UL))->APB1ENR &= ~((0x1UL << (0U))));
    HAL_GPIO_DeInit(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0000UL)), ((uint16_t)0x0020));
}
...
static void tpad_reset(void)
{
    ...
    GPIO_InitStruct.Pin = ((uint16_t)0x0020);
    GPIO_InitStruct.Mode = (0x00000000U | 0x00000001U);
    GPIO_InitStruct.Pull = 0x00000002U;
    GPIO_InitStruct.Speed = 0x00000002U;
    HAL_GPIO_Init(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0000UL)), &GPIO_InitStruct);
    HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0000UL)), ((uint16_t)0x0020), GPIO_PIN_RESET);
    delay_ms(10);
...
static uint16_t tpad_get_val(void)
{
    ...
    while((((&tim_cap_handler)->Instance->SR &((0x1UL << (1U)))) == ((0x1UL << (1U)))) == RESET)
    {
        if((tim_cap_handler.Instance->CNT > 0xFFFFFFFF - 500) ||
           (timeout++ > max_timeout))
        {
...
uint8_t tpad_scan(uint8_t mode)
{
    ...
    uint16_t val = tpad_get_max(5);

    if(val > (tpad_default_val + 50))
    {
...
uint8_t tpad_init(uint16_t psc)
{
    ...
    const uint8_t start_idx = (20 - 10) / 2;
    for(uint8_t i = start_idx; i < start_idx + 10; i++)
    {
        temp += buf[i];
    }
    tpad_default_val = temp / 10;

    printf("TPAD Default Value: %u\r\n", tpad_default_val);


    if(tpad_default_val > 0xFFFFFFFF / 2)
    {
