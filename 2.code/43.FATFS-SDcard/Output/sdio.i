# 1 "../User/BSP/sdio/sdio.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 387 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../User/BSP/sdio/sdio.c" 2
...
{
    uint8_t SD_status;
    sdcard_handle.Instance = ((SDIO_TypeDef *) ((0x40000000UL + 0x00010000UL) + 0x2C00UL));
    sdcard_handle.Init.ClockEdge = 0x00000000U;
    sdcard_handle.Init.ClockPowerSave = 0x00000000U;
    sdcard_handle.Init.ClockBypass = 0x00000000U;
    sdcard_handle.Init.BusWide = 0x00000000U;
    sdcard_handle.Init.HardwareFlowControl = 0x00000000U;





    sdcard_handle.Init.ClockDiv = 1;
    SD_status = HAL_SD_Init(&sdcard_handle);
    if(SD_status != HAL_OK)
    {
        return 1;
    }
    HAL_SD_GetCardInfo(&sdcard_handle, &sdcard_info_handle);
    SD_status = HAL_SD_ConfigWideBusOperation(&sdcard_handle, (0x1UL << (11U)));
    {
        return 2;
    }
    return 0;
...
void HAL_SD_MspInit(SD_HandleTypeDef *hsd)
{
    ...
    do { volatile uint32_t tmpreg = 0x00U; ((((RCC_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x3800UL))->APB2ENR) |= ((0x1UL << (11U)))); tmpreg = ((((RCC_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x3800UL))->APB2ENR) & ((0x1UL << (11U)))); (void)tmpreg; } while(0U);
    ...
    GPIO_Initure.Pin = ((uint16_t)0x0100)|((uint16_t)0x0200)|((uint16_t)0x0400)|((uint16_t)0x0800)|((uint16_t)0x1000);
    ...
    HAL_GPIO_Init(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0800UL)), &GPIO_Initure);
    GPIO_Initure.Pin = ((uint16_t)0x0004);
    GPIO_Initure.Alternate = ((uint8_t)0x0C);
...
uint8_t get_sd_card_state(void)
{
    return ((HAL_SD_GetCardState(&sdcard_handle) == 0x00000004U) ? ((uint8_t)0x00) : ((uint8_t)0x01));
}
# 85 "../User/BSP/sdio/sdio.c"
uint8_t sd_read_disk(uint8_t *pbuf, uint32_t saddr, uint32_t cnt)
{
    uint8_t sta = HAL_OK;
    ...
    __disable_irq();
    sta = HAL_SD_ReadBlocks(&sdcard_handle, (uint8_t *)pbuf, lsector, cnt, ((uint32_t)100000000));

    while (get_sd_card_state() != ((uint8_t)0x00))
    {
        if (timeout-- == 0)
        {
            sta = ((uint8_t)0x01);
...
uint8_t sd_write_disk(uint8_t *pbuf, uint32_t saddr, uint32_t cnt)
{
    ...
    long long lsector = saddr;
    __disable_irq();
    sta = HAL_SD_WriteBlocks(&sdcard_handle, (uint8_t *)pbuf, lsector, cnt, ((uint32_t)100000000));
