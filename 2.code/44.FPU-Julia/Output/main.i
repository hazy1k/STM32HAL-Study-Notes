...
# 1 "../User/BSP/tim\\tim.h" 1
# 10 "../User/BSP/tim\\tim.h"
...
void julia_clut_init(uint16_t *clut)
{
    ...
    for (i = 0; i < 128; i++)
    {
        ...
        red = (i * 8 * 256 / 128) % 256;
        green = (i * 6 * 256 / 128) % 256;
        blue = (i * 4 * 256 / 128) % 256;
...
void julia_generate_fpu(uint16_t size_x, uint16_t size_y, uint16_t offset_x, uint16_t offset_y, uint16_t zoom)
{
    ...
    for (y = 0; y < size_y; y++)
    {
        for (x = 0; x < size_x; x++)
        {
            ...
            while ((i < 128 - 1) && (radius < 4))
            {
                tmp1 = num_real * num_real;
                tmp2 = num_img * num_img;
                num_img = 2 * num_real * num_img + 0.01f;
                num_real = tmp1 - tmp2 + 0.285f;
                radius = tmp1 + tmp2;
                i++;
            }
            ((LCD_TypeDef *) ((uint32_t)(0x6C000000 | 0x0000007E)))->LCD_RAM = g_color_map[i];
        }
    }
}
...
int main(void)
{
    ...
    while(1)
 {
        ...
        ((TIM_TypeDef *) (0x40000000UL + 0x1000UL))->CNT = 0;
        timeout = 0;
        julia_generate_fpu(lcddev.width, lcddev.height, lcddev.width/2, lcddev.height/2, zoom_ratio[i]);
        time = ((TIM_TypeDef *) (0x40000000UL + 0x1000UL))->CNT + (uint32_t)timeout * 65536;
        sprintf(buf, "%s: Zoom:%d  RunTime:%0.1fms", "FPU On",zoom_ratio[i], time/10);
