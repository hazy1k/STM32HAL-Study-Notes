# 1 "../User/BSP/stmflash/stmflash.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 387 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../User/BSP/stmflash/stmflash.c" 2
...
# 5 "../User/BSP/stmflash/stmflash.h" 2
...
# 2 "../User/BSP/stmflash/stmflash.c" 2
...
void delay_us(uint32_t nus);
    ...
    void HAL_Delay(uint32_t Delay);
# 3 "../User/BSP/stmflash/stmflash.c" 2
...
uint8_t stmflash_get_flash_sector(uint32_t addr)
{
    if (addr < ((uint32_t )0x08004000)) return 0U;
    else if (addr < ((uint32_t )0x08008000)) return 1U;
    else if (addr < ((uint32_t )0x0800C000)) return 2U;
    else if (addr < ((uint32_t )0x08010000)) return 3U;
    else if (addr < ((uint32_t )0x08020000)) return 4U;
    else if (addr < ((uint32_t )0x08040000)) return 5U;
    else if (addr < ((uint32_t )0x08060000)) return 6U;
    else if (addr < ((uint32_t )0x08080000)) return 7U;
    else if (addr < ((uint32_t )0x080A0000)) return 8U;
    else if (addr < ((uint32_t )0x080C0000)) return 9U;
    else if (addr < ((uint32_t )0x080E0000)) return 10U;
    return 11U;
}
...
static uint32_t get_sector_size(uint8_t sector)
{
    ...
    else if (sector == 4) return 64 * 1024;
    else return 128 * 1024;
}
# 59 "../User/BSP/stmflash/stmflash.c"
void stmflash_write(uint32_t waddr, uint32_t *pbuf, uint32_t length)
{
    ...
    if(waddr < 0X08000000 || waddr % 4 || waddr > (0X08000000 + 0x100000)){
        return;
    }

    HAL_FLASH_Unlock();
    ((FLASH_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x3C00UL))->ACR &= ~(1<<10);
    ...
    if(addrx < 0x1FFF0000)
    {
        while(addrx < endaddr)
        {
            ...
            if(stmflash_read_word(addrx) != 0xFFFFFFFF)
            {
                ...
                addrx = ((uint32_t )0x08000000) + (sector + 1) * sector_size;
            }
            else
            {
                addrx += 4;
    ...
    if(FlashStatus == HAL_OK)
    {
        while(waddr < endaddr)
        {
            if(HAL_FLASH_Program(0x00000002U, waddr, *pbuf) != HAL_OK)
            {
                break;
            }
            waddr += 4;
...
# 134 "../User/BSP/stmflash/stmflash.c"
void stmflash_read(uint32_t raddr, uint32_t *pbuf, uint32_t length)
{
    uint32_t i;
