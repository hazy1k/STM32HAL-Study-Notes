# 1 "../Middlewares/FATFS/exfuns/exfuns.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 387 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../Middlewares/FATFS/exfuns/exfuns.c" 2
...
# 2 "../Middlewares/FATFS/exfuns/exfuns.c" 2
...
# 3 "../Middlewares/FATFS/exfuns/exfuns.c" 2
...
# 4 "../Middlewares/FATFS/exfuns/exfuns.c" 2
...
# 5 "../Middlewares/FATFS/exfuns/exfuns.c" 2
# 1 "../Middlewares/FATFS/exfuns/fattester.h" 1
# 18 "../Middlewares/FATFS/exfuns/fattester.h"
typedef struct
{
    FIL *file;
    FILINFO fileinfo;
...
# 6 "../Middlewares/FATFS/exfuns/exfuns.c" 2





char *const FILE_TYPE_TBL[7][7] =
{
    {"BIN"},
    {"LRC"},
    {"NES", "SMS"},
...
FATFS *fs[2];
# 36 "../Middlewares/FATFS/exfuns/exfuns.c"
uint8_t exfuns_init(void)
{
    uint8_t i;
    uint8_t res = 0;
...
# 88 "../Middlewares/FATFS/exfuns/exfuns.c"
uint8_t exfuns_file_type(char *fname)
{
    uint8_t tbuf[5];
    char *attr = 0;
...
# 146 "../Middlewares/FATFS/exfuns/exfuns.c"
uint8_t exfuns_get_free(uint8_t *pdrv, uint32_t *total, uint32_t *free)
{
    FATFS *fs1;
    uint8_t res;
...
# 198 "../Middlewares/FATFS/exfuns/exfuns.c"
                                      ...
                                      uint32_t totsize, uint32_t cpdsize, uint8_t fwmode)
{
    ...
    fsrc = (FIL *)mymalloc(0, sizeof(FIL));
    fdst = (FIL *)mymalloc(0, sizeof(FIL));
    fbuf = (uint8_t *)mymalloc(0, 8192);

    if (fsrc == 0 || fdst == 0 || fbuf == 0)
    {
        res = 100;
    }
    else
    {
        ...
        res = f_open(fsrc, (const TCHAR *)psrc, 0x01 | 0x00);

        if (res == 0)res = f_open(fdst, (const TCHAR *)pdst, 0x02 | fwmode);
...
# 289 "../Middlewares/FATFS/exfuns/exfuns.c"
...
# 313 "../Middlewares/FATFS/exfuns/exfuns.c"
uint32_t exfuns_get_folder_size(uint8_t *fdname)
{
    ...
    fddir = (DIR *)mymalloc(0, sizeof(DIR));
    finfo = (FILINFO *)mymalloc(0, sizeof(FILINFO));

    if (fddir == 0 || finfo == 0)res = 100;

    if (res == 0)
    {
        pathname = mymalloc(0, 512 + 1);

        if (pathname == 0)res = 101;
...
# 413 "../Middlewares/FATFS/exfuns/exfuns.c"
                           ...
                           uint32_t *totsize, uint32_t *cpdsize, uint8_t fwmode)
{
    ...
    srcdir = (DIR *)mymalloc(0, sizeof(DIR));
    dstdir = (DIR *)mymalloc(0, sizeof(DIR));
    finfo = (FILINFO *)mymalloc(0, sizeof(FILINFO));

    if (srcdir == 0 || dstdir == 0 || finfo == 0)res = 100;

    if (res == 0)
    {
        dstpathname = mymalloc(0, 512 + 1);
        srcpathname = mymalloc(0, 512 + 1);

        if (dstpathname == 0 || srcpathname == 0)res = 101;
        ...
        if (res == 0)
        {
            ...
            myfree(0, dstpathname);
            myfree(0, srcpathname);
        }
    }
