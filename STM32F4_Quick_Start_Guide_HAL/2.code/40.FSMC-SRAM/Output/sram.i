# 1 "../User/BSP/sram/sram.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 387 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../User/BSP/sram/sram.c" 2
...
void sram_init(void)
{
    ...
    GPIO_InitStructure.Alternate = ((uint8_t)0x0C);
    HAL_GPIO_Init(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x1800UL)), &GPIO_InitStructure);
    GPIO_InitStructure.Pin = ((uint16_t)0x0020)|((uint16_t)0x0010);
    HAL_GPIO_Init(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0C00UL)), &GPIO_InitStructure);

    GPIO_InitStructure.Pin = ((uint16_t)0x0001)|((uint16_t)0x0002)|((uint16_t)0x0010)|((uint16_t)0x0020)|((uint16_t)0x0100)|((uint16_t)0x0200)|((uint16_t)0x0400)|((uint16_t)0x0800)|((uint16_t)0x1000)|((uint16_t)0x2000)|((uint16_t)0x4000)|((uint16_t)0x8000);
    GPIO_InitStructure.Mode = (0x00000000U | 0x00000002U);
    GPIO_InitStructure.Pull = 0x00000001U;
    GPIO_InitStructure.Speed = 0x00000003U;
    HAL_GPIO_Init(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x0C00UL)), &GPIO_InitStructure);

    GPIO_InitStructure.Pin = ((uint16_t)0x0001)|((uint16_t)0x0002)|((uint16_t)0x0080)|((uint16_t)0x0100)|((uint16_t)0x0200)|((uint16_t)0x0400)|((uint16_t)0x0800)|((uint16_t)0x1000)|((uint16_t)0x2000)|((uint16_t)0x4000)|((uint16_t)0x8000);
    HAL_GPIO_Init(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x1000UL)), &GPIO_InitStructure);

    GPIO_InitStructure.Pin = ((uint16_t)0x0001)|((uint16_t)0x0002)|((uint16_t)0x0004)|((uint16_t)0x0008)|((uint16_t)0x0010)|((uint16_t)0x0020)|((uint16_t)0x1000)|((uint16_t)0x2000)|((uint16_t)0x4000)|((uint16_t)0x8000);
    HAL_GPIO_Init(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x1400UL)), &GPIO_InitStructure);

    GPIO_InitStructure.Pin = ((uint16_t)0x0001)|((uint16_t)0x0002)|((uint16_t)0x0004)|((uint16_t)0x0008)|((uint16_t)0x0010)|((uint16_t)0x0020)|((uint16_t)0x0400);
    HAL_GPIO_Init(((GPIO_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x1800UL)), &GPIO_InitStructure);

    sram_handle.Instance = ((FSMC_Bank1_TypeDef *) (0xA0000000UL + 0x0000UL));
    sram_handle.Extended = ((FSMC_Bank1E_TypeDef *) (0xA0000000UL + 0x0104UL));
    sram_handle.Init.NSBank = (3 == 1) ? 0x00000000U : (3 == 2) ? 0x00000002U : (3 == 3) ? 0x00000004U : 0x00000006U;


    sram_handle.Init.DataAddressMux = 0x00000000U;
    sram_handle.Init.MemoryType = 0x00000000U;
    sram_handle.Init.MemoryDataWidth = 0x00000010U;
    sram_handle.Init.BurstAccessMode = 0x00000000U;
    sram_handle.Init.WaitSignalPolarity = 0x00000000U;
    sram_handle.Init.WaitSignalActive = 0x00000000U;
    sram_handle.Init.WriteOperation = 0x00001000U;
    sram_handle.Init.WaitSignal = 0x00000000U;
    sram_handle.Init.ExtendedMode = 0x00000000U;
    sram_handle.Init.AsynchronousWait = 0x00000000U;
    sram_handle.Init.WriteBurst = 0x00000000U;

    fsmc_handle.AddressSetupTime = 0x02;
    fsmc_handle.AddressHoldTime = 0x00;
    fsmc_handle.DataSetupTime = 0x08;
    fsmc_handle.BusTurnAroundDuration = 0x00;
    fsmc_handle.AccessMode = 0x00000000U;
    HAL_SRAM_Init(&sram_handle, &fsmc_handle, &fsmc_handle);
}
# 71 "../User/BSP/sram/sram.c"
void sram_write(uint8_t *pbuf, uint32_t addr, uint32_t datalen)
{
    for (; datalen != 0; datalen--)
    {
        *(volatile uint8_t *)((0X60000000 + (0X4000000 * (3 - 1))) + addr) = *pbuf;
        addr++;
        pbuf++;
    }
}
# 88 "../User/BSP/sram/sram.c"
void sram_read(uint8_t *pbuf, uint32_t addr, uint32_t datalen)
{
    for (; datalen != 0; datalen--)
    {
        *pbuf++ = *(volatile uint8_t *)((0X60000000 + (0X4000000 * (3 - 1))) + addr);
        addr++;
    }
}
# 104 "../User/BSP/sram/sram.c"
void sram_test_write(uint32_t addr, uint8_t data)
{
