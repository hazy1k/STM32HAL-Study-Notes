# 1 "../Middlewares/SerialInvoker.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 387 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../Middlewares/SerialInvoker.c" 2
...
# 2 "../Middlewares/SerialInvoker.c" 2
...
# 3 "../Middlewares/SerialInvoker.c" 2
...
# 5 "../Middlewares/SerialInvoker.c" 2
...
uint8_t serial_invoker_sys_cmd_exe(char *str) {
    ...
    res = serial_invoker_get_cmdname(str, sfname, &i, 30);
    if (res) return 1;
    str += i;
    ...
    switch (i) {
        ...
        case 1:
            ...
            printf("------------------------SerialInvoker V1.0------------------------ \r\n");
            printf("SerialInvoker是由Hazy1k开发的灵巧串口调试交互组件,通过它你可以通过串口助手调用程序中的任何函数并执行.\r\n");
            printf("因此,你可以随意更改函数输入参数(支持数字/字符串/函数指针)\r\n");
            printf("单个函数最多支持10个输入参数,并支持函数返回值显示.支持参数显示进制设置及转换功能.\r\n");
            printf("技术支持:https://www.cnblogs.com/hazy1k\r\n");
            printf("SerialInvoker有7个系统命令(必须小写):\r\n");
            printf("?:      获取帮助信息\r\n");
            printf("help:   获取帮助信息\r\n");
            printf("list:   可用的函数列表\r\n");
            printf("id:     可用函数的ID列表\r\n");
            printf("hex:    参数16进制显示,后跟数字即执行进制转换\r\n");
            printf("dec:    参数10进制显示,后跟数字即执行进制转换\r\n");
            printf("runtime:1,开启函数运行计时;0,关闭函数运行计时;\r\n");
            printf("请按照函数格式输入函数名及参数并以回车键结束.\r\n");
            printf("-------------------------------------------------------------- \r\n");
...
uint8_t serial_invoker_cmd_rec(char *str) {
    ...
    if (i == serial_invoker_dev.fnum) return 4;
...
void serial_invoker_exe(void) {
    ...
    for (i = 0; i < pnum; i++) {
        if (serial_invoker_dev.parmtype & (1 << i)) {
            printf("\"%s\"", serial_invoker_dev.parm + serial_invoker_get_parmpos(i));
            temp[i] = (uint32_t)&(serial_invoker_dev.parm[serial_invoker_get_parmpos(i)]);
        }
        else {
            temp[i] = *(uint32_t *)(serial_invoker_dev.parm + serial_invoker_get_parmpos(i));
            if (serial_invoker_dev.sptype == 0) {
...
void serial_invoker_timx_reset_time(void) {
    ((&g_timx_serial_invoker_handle)->Instance->SR = ~((0x1UL << (0U))));
    do{ (&g_timx_serial_invoker_handle)->Instance->ARR = (0XFFFF); (&g_timx_serial_invoker_handle)->Init.Period = (0XFFFF); } while(0);
    ((&g_timx_serial_invoker_handle)->Instance->CNT = (0));
    serial_invoker_dev.runtime = 0;
}
uint32_t serial_invoker_timx_get_time(void) {
    if ((((&g_timx_serial_invoker_handle)->Instance->SR &((0x1UL << (0U)))) == ((0x1UL << (0U)))) == SET) {
        serial_invoker_dev.runtime += 0XFFFF;
    }
    serial_invoker_dev.runtime += ((&g_timx_serial_invoker_handle)->Instance->CNT);
    return serial_invoker_dev.runtime;
}
void serial_invoker_timx_init(uint16_t arr, uint16_t psc) {
    do{ do { volatile uint32_t tmpreg = 0x00U; ((((RCC_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x3800UL))->APB1ENR) |= ((0x1UL << (2U)))); tmpreg = ((((RCC_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x3800UL))->APB1ENR) & ((0x1UL << (2U)))); (void)tmpreg; } while(0U); }while(0);
    g_timx_serial_invoker_handle.Instance = ((TIM_TypeDef *) (0x40000000UL + 0x0800UL));
    g_timx_serial_invoker_handle.Init.Prescaler = psc;
    g_timx_serial_invoker_handle.Init.CounterMode = 0x00000000U;
    g_timx_serial_invoker_handle.Init.Period = arr;
    g_timx_serial_invoker_handle.Init.ClockDivision = 0x00000000U;
...
void TIM4_IRQHandler(void) {
    if (((((&g_timx_serial_invoker_handle)->Instance->DIER & ((0x1UL << (0U)))) == ((0x1UL << (0U)))) ? SET : RESET) == SET) {
        serial_invoker_dev.scan();
        ((&g_timx_serial_invoker_handle)->Instance->CNT = (0));
        do{ (&g_timx_serial_invoker_handle)->Instance->ARR = (100); (&g_timx_serial_invoker_handle)->Init.Period = (100); } while(0);
    }
