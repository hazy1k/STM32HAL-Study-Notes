...
{    
    if(huart->Instance == USART1) // 如果是串口 1
        ...
        {
            if(USART_RX_STA & 0x4000) // 接收到了 0x0d，也就是回车字符
            {   
                if(aRxBuffer[0] != 0x0a) // 如果没有接收到换行符，表示接收错误
                    USART_RX_STA = 0; // 接收错误,重新开始
                else USART_RX_STA |= 0x8000; // 接收完成了
            }
            else // 还没收到 0X0D，也就是回车字符
            {
                if(aRxBuffer[0] == 0x0d) // 如果当前接收到了回车字符
                    USART_RX_STA |= 0x4000; // 更新状态
                else
                {
                    // 没有接收到回车字符，将数据存入缓冲区
                    USART_RX_BUF[USART_RX_STA & 0X3FFF] = aRxBuffer[0];
                    USART_RX_STA++; // 标志位自增
                    if(USART_RX_STA > (USART_REC_LEN-1)) // 超过最大限度
...
1. **函数定义**：
   ...
   - `void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)`：这是由 HAL 库提供的一个回调函数，用于处理 UART 接收完成的事件。当 UART 接收到数据并完成接收时，这个函数会被自动调用。

2. **实例判断**：
   
   - `if(huart->Instance == USART1)`：这里检查传入的 UART 实例是否为 `USART1`。如果不是，则不进行任何操作。这确保了只有特定的 UART 被处理。

3. **接收状态检查**：
   
   - `if((USART_RX_STA & 0x8000) == 0)`：这个条件检查接收状态是否完成。`0x8000` 是用来标记接收是否完成的标志位。
     - 如果 `USART_RX_STA` 的高位（第15位）为0，说明接收尚未完成，程序继续处理接收逻辑。

4. **已收到 0x0D 的判断**：
   
   - `if(USART_RX_STA & 0x4000)`：这个条件判断是否已经接收到回车字符 `0x0D`。`0x4000` 是用来标记是否收到 `0x0D` 的标志位。

5. **接收完成判断**：
   
   - 如果已收到 `0x0D`，接着检查接收到的下一个字符：
     - `if(aRxBuffer[0] != 0x0A)`：如果接收到的字符不是换行符 `0x0A`，则表示接收错误，重置状态 `USART_RX_STA = 0`。
     - 如果是 `0x0A`，则设置接收完成标志 `USART_RX_STA |= 0x8000`。

6. **未收到 0x0D 的处理**：
   
   - 如果尚未收到 `0x0D`，程序继续接收数据：
     - `if(aRxBuffer[0] == 0x0D)`：如果当前接收到的字符是 `0x0D`，则更新状态 `USART_RX_STA |= 0x4000`，表示接收到回车。
     - 否则，将接收到的字符存入接收缓冲区 `USART_RX_BUF`：

```c
USART_RX_BUF[USART_RX_STA & 0x3FFF] = aRxBuffer[0];
```

`USART_RX_STA & 0x3FFF` 确保索引在有效范围内（0-16383），避免数组越界。

7. **接收状态更新与溢出检查**：
- 每次接收到新字符后，状态计数器 `USART_RX_STA++` 自增。
- `if(USART_RX_STA > (USART_REC_LEN - 1))`：此条件检查接收状态是否超过了预定义的最大接收长度 `USART_REC_LEN`，如果超过，则重置状态 `USART_RX_STA = 0`，表示接收发生错误。
...
`HAL_UART_Receive()` 是 STM32 HAL 库中用于接收数据的函数。它可以通过轮询、DMA 或中断等方式从 UART 外设接收数据，并将接收到的数据存储在指定的缓冲区中。
...
1. **UART_HandleTypeDef *huart**:
   ...
   - 指向 UART 句柄的指针，包含 UART 配置和状态信息。这个句柄是在初始化 UART 时创建的。

2. **uint8_t *pData**:
   
   - 指向接收数据的缓冲区，接收到的数据将被存储在此地址。
...
4. **uint32_t Timeout**:
   ...
   - 接收超时时间，以毫秒为单位。如果在指定时间内未接收到预期数量的数据，函数将返回超时错误。

返回类型为 `HAL_StatusTypeDef`，可能的返回值包括：
...
// 接收 10 字节数据，超时时间为 1000 毫秒
status = HAL_UART_Receive(&huart1, rxBuffer, 10, 1000);

if (status == HAL_OK) {
    // 数据接收成功，可以处理 rxBuffer 中的数据
...
`HAL_UART_Transmit()` 是 STM32 HAL 库中用于通过 UART 发送数据的函数。该函数可以以阻塞模式发送数据。
...
1. **UART_HandleTypeDef *huart**:
   ...
   - 指向 UART 句柄的指针，包含 UART 的配置和状态信息。

2. **uint8_t *pData**:
   
   - 指向要发送的数据缓冲区的指针，数据将从这个缓冲区中读取。
...
4. **uint32_t Timeout**:
   ...
   - 发送超时时间，以毫秒为单位。如果在指定时间内未完成发送，函数将返回超时错误。
- 返回类型为 `HAL_StatusTypeDef`，可能的返回值包括：
  - `HAL_OK`: 发送成功。
  - `HAL_TIMEOUT`: 发送超时。
  - `HAL_ERROR`: 发生错误。
...
// 发送数据，超时时间为 1000 毫秒
status = HAL_UART_Transmit(&huart1, dataToSend, sizeof(dataToSend), 1000);
...
如果我们想要使用串口接发实际上很容易，但是我们代码使用一个协议来保证数据传输可靠性，看起来比较麻烦。下面我们可以简单总结一下一般的情况
...
- USB 转 TTL 模块
- 跳线
- PC（安装终端软件，如 PuTTY 或 Tera Term）

### 硬件连接

1. 将 USB 转 TTL 模块的 **TX** 引脚连接到开发板的 **PA10**（USART1 RX）。
2. 将模块的 **RX** 引脚连接到开发板的 **PA9**（USART1 TX）。
3. 将模块的 **GND** 引脚连接到开发板的 **GND** 引脚。
...
UART_HandleTypeDef huart1; // 声明 USART1 句柄
...
static void MX_USART1_UART_Init(void)
{
    huart1.Instance = USART1; // 设置 USART1 实例
    ...
    huart1.Init.Mode = UART_MODE_TX_RX; // 设置为发送接收模式
...
void SystemClock_Config(void)
{
    ...
    HAL_RCC_OscConfig(&RCC_OscInitStruct); // 配置振荡器
    ...
    HAL_RCC_ClockConfig(&RCC_ClkInitStruct, FLASH_LATENCY_5); // 配置时钟
}
